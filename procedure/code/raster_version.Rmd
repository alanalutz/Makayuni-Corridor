...
```{r}
# From Malcomb reproduction
b = st_bbox(st_bbox(lc_study_clip_reproj), crs=st_crs(32736)) %>% st_as_sfc()

blank <- st_as_stars(b, dx=10, dy=10)
blank[[1]][]=NA

secondary_roads_raster <- st_rasterize(output_1a, template=blank)
#write_stars(secondary_roads_raster, "secondary_roads_raster.tif")
...
rural_raster <- st_rasterize(rural_reproj, template=blank)
...
buildings_raster <- st_rasterize(buildings_reproj, template=blank)
